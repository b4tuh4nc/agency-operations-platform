...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      // Admin ve Computer Manager için SLA durumunu getir
      if (parsedUser.role === 'admin' || parsedUser.role === 'computer_manager') {
        fetchSlaStatus();
      }
    }
  }, []);
  ...
  const fetchSlaStatus = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3000/uptime/stats?period=7d', {
      ...
      console.error('SLA status fetch error:', error);
    }
  };
      ...
      <div className="flex-1 p-8">
        <div className="mb-8">
          ...
          <p className="text-slate-600">
            {getRoleDisplayName(user.role)} - AdManager Pro'ya hoş geldiniz
          </p>
        </div>

        {/* SLA Alert - Admin ve Computer Manager için */}
        {(user?.role === 'admin' || user?.role === 'computer_manager') && slaStatus && slaStatus.sla?.status === 'violated' && (
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-red-800 mb-1">SLA İhlali Tespit Edildi!</h3>
                  <p className="text-sm text-red-700 mb-2">
                    Haftalık kesinti limiti aşıldı. Sistem {Math.floor(slaStatus.sla.used / 60)} dakika kesinti yaşadı (Limit: 60 dakika).
                  </p>
                  <Link 
                    href="/admin/uptime"
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            >
              ...
              <div>
                <h3 className="font-medium text-slate-800">Kullanıcı Yönetimi</h3>
                <p className="text-sm text-slate-600">Kullanıcıları görüntüle ve düzenle</p>
              </div>
            </Link>
          </div>
        </div>
