...
export default function MyTasks() {
  ...
  const fetchMyTasks = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Görevler alındı:', data);
        setTasks(data);
      } else {
        console.error('Görevler alınamadı:', response.status);
      }
    } catch (error) {
  ...
  const getPriorityBadge = (priority: string) => {
    ...
    return badges[priority] || 'bg-gray-50 text-gray-600 border-gray-200';
  };
              ...
              <div key={task._id} className="bg-white rounded-xl p-6 shadow-sm border border-slate-200 hover:shadow-md transition-shadow">
                    ...
                    <div className="flex items-center space-x-4 text-sm text-slate-500">
                      {task.campaign && (
                        <>
                          <span className="flex items-center">
                            ...
                            Kampanya: {task.campaign?.title || 'Bilinmiyor'}
                          </span>
                          <span>•</span>
                        </>
                      )}
                      {task.advert && (
                        <>
                          <span className="flex items-center">
                            ...
                            Reklam: {task.advert?.title || 'Bilinmiyor'}
                          </span>
                          <span>•</span>
                        </>
                      )}
                      {task.assignedBy && (
                        <span className="flex items-center">
                          ...
                          Atayan: {task.assignedBy?.firstName || ''} {task.assignedBy?.lastName || ''} {!task.assignedBy?.firstName && !task.assignedBy?.lastName && (task.assignedBy?.email || 'Bilinmiyor')}
                        </span>
                      )}
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    disabled={task.status === 'submitted' || task.status === 'completed' || task.status === 'approved'}
                  />
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {task.uploadedFiles.map((file: string, idx: number) => (
                        <button
                          ...
                          className="text-left p-3 bg-white rounded-lg border border-green-200 hover:border-green-400 hover:bg-green-50 transition-all group"
                        >
                          <div className="flex items-center space-x-2">
                            <svg className="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                            </svg>
                            <span className="text-sm text-green-700 group-hover:text-green-800 font-medium truncate">
                ...
                {task.status !== 'submitted' && task.status !== 'completed' && task.status !== 'approved' && (
                  <button
                    onClick={() => handleSubmitClick(task._id, task.title || 'Görev')}
